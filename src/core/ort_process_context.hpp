...
class OrtProcessContext {
   ...
   private:
    ...
    std::mutex m_mutex;
    std::optional<Ort::ThreadingOptions> m_threading_options = std::nullopt;
    std::optional<Ort::Env> m_env = std::nullopt;
    std::optional<Ort::ArenaCfg> m_cpu_arena_cfg = std::nullopt;
    std::optional<Ort::MemoryInfo> m_cpu_memory_info = std::nullopt;
